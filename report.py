    ...
    while (len([e for e in force if e in erreurs]) or succes == []) and t <= 2:
        ...
        for spot in spots:
            try: 
                ...
                if t == 0:  erreurs += [spot]
            print('\r',end='\r')
            print(len(succes),' succès et ',len(erreurs),' echec(s)',end='\r')
        for s in succes : 
...
def f_lit(loc):
    coor = loc.copy()
    lit = pd.read_csv('litoral.csv',sep=';'); lit['dist'] = np.cumsum([0]+[np.sqrt((lit['lat'][i-1]-lit['lat'][i])**2 + (lit['long'][i-1]-lit['long'][i])**2) for i in list(lit.index)[1:]]); lit['dist'] = lit['dist']/max(lit['dist'])       
...
def f_post(spot_f,prev,Max=1e6):
    ...
    img_path= ['C:\\','Users','Lenovo-x1','Documents','GitHub','mirmoc','temp.png']
    fig.savefig(os.path.join(*img_path), dpi=100)
    #plt.close('all')
    return prev,spot_f

def push(wait=300,path = ['C:\\','Users','Lenovo-x1','Documents','GitHub','mirmoc']):
    os.chdir(os.path.join(*path))
    run=['git add temp.png -f','git commit -a -m "maj"','git push']
    for c in run: os.system(c)
    time.sleep(wait)
    
def send_mail(adresses=list(pd.read_csv(os.path.join(os.getcwd(),'maillist.csv'),sep=';')['adresse'])):
    img_path = os.path.join(os.getcwd(),'temp.png')
    img_data = open(img_path, 'rb').read()
    for to in adresses:
        msg = MIMEMultipart()
        msg['Subject'] = 'Report'
        msg['From'] = 'theoxdorlin@gmail.com'
        msg['To'] = to
        text = MIMEText("With love"); msg.attach(text)
        image = MIMEImage(img_data, name=os.path.basename(img_path))
        msg.attach(image)
        s = smtplib.SMTP('smtp.gmail.com', 587); s.ehlo(); s.starttls(); s.ehlo()
        s.login(msg['From'], 'cpttp:)78ef')
        s.sendmail(msg['From'] , msg['To'], msg.as_string()); s.quit()

def send_sms(num='+33682444721'):    
